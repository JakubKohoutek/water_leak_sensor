...
print_usage() {
  cat <<'EOF'
Usage:
  scripts/upload.sh [--compile|-c]

Examples:
  scripts/upload.sh
  scripts/upload.sh -c
  scripts/upload.sh --compile
EOF
}
...
if [ "$do_compile" = true ]; then
  ...
  arduino-cli compile --fqbn "$FQBN" --libraries ~/Documents/Arduino/libraries .
fi

if [ "${#ports[@]}" -eq 0 ]; then
  echo "No upload-capable serial devices detected."
  exit 1
fi
...
arduino-cli upload --fqbn "$FQBN" --port "$selected_port" .

echo "Upload complete. Starting serial monitor on ${selected_port} (baud ${BAUD})."
